...
class Seq2SeqModel(object):
    ...
    def get_batch_data1(self, data, bucket_id):
        ...
        encoder_size, decoder_size = self.buckets[bucket_id]
        encoder_inputs, decoder_inputs, bl_decoder, decoder_emotions = [], [], [], []
        ...
        for idx in xrange(self.batch_size):
            ...
            # Decoder inputs get an extra "GO" symbol, and are padded then.
            bl_decoder.append(decoder_input)
            decoder_pad_size = decoder_size - len(decoder_input) - 1
            decoder_inputs.append([data_utils.GO_ID] + decoder_input +
                                  [data_utils.PAD_ID] * decoder_pad_size)
        ...
        for length_idx in xrange(decoder_size):
            ...
            for batch_idx in xrange(self.batch_size):
                ...
                if length_idx == decoder_size - 1 or target == data_utils.PAD_ID:
                    batch_weight[batch_idx] = 0.0
            batch_weights.append(batch_weight)
        return batch_encoder_inputs, batch_decoder_inputs, batch_weights, batch_decoder_emotions, bl_decoder
